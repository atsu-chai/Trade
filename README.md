...
- 通常監視 / 強監視 / 停止の管理
- 保有単価、保有株数、買い増し設定の管理
- Yahoo Financeによる日足OHLCVデータ取得
  - 銘柄一覧と詳細の最新価格はYahoo Financeの `regularMarketPrice` を使用
...
Supabase Edge Function版は、既定ではYahoo Financeの日足データを使います。認証情報は不要です。
...
J-Quants APIを使う場合だけ、Providerと認証情報を設定してください。
